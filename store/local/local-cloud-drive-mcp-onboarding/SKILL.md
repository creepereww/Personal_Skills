...
description: 给 opencode（或同类 MCP 客户端）接入一个新的网盘/云盘 MCP 时使用。覆盖"查官方有无 → 判断凭证类型 → 官方付费时自建免费路线 → 端点探测 → 登录 → 三件套验证"的完整流程，含 123云盘站内接口速查、跨平台查重禁忌（不能用 md5）、秒传探测的副作用、分页陷阱与看门狗，以及**长跑搬运的后台进程托管（Windows Job Object 逃逸 / WMI 启动 / 无窗口）**。触发词：装个 X 网盘 MCP、接入网盘 MCP、云盘 MCP、自建 MCP 服务端、网盘接口失效、跨盘查重、秒传、搬运进程被杀、后台任务自动停了。
agent_created: true
version: v1.0
---
...
1. **有没有官方 MCP**：`WebSearch` + 直接查仓库注册表（比搜索准）：
   ...
   ```
   GitHub 匿名搜索 API 很容易限流，返回 `total_count: 0` 时先怀疑被限流，换 `raw.githubusercontent.com` 直接取文件。
2. **官方 MCP 要钱吗**：很多国内网盘的 OpenAPI 已改付费（如 123云盘需买"开发者权益包"，且要 VIP 身份）。
   先搜官方文档确认，再决定：付费走官方 / 免费走站内接口自建。
3. **凭证形态 = 是否可信的信号**：
...
- 兜底方案：账号密码登录（`POST /api/user/sign_in`，`{"type":1,"passport":...,"password":...}`，code=200 成功）
...
2. stdio 真实调用一次：管道喂 `initialize` / `notifications/initialized` / `tools/call`
   （注意：Git Bash 管道传中文可能 UnicodeDecodeError，改用 PowerShell 写文件再 Python 读，utf-16 解码）
...
- `POST /b/api/file/upload_request` `{driveId:0, etag:<md5>, fileName, parentFileId, size, type:0, duplicate:0}`
  → 返回 `Reuse:true` 时是**秒传**，服务端池里已有，无需再传；否则回 `Bucket/Key/UploadId/StorageNode/FileId`
  → **只给 etag+size+fileName 就能凭空生成文件**（实测 0.24 秒生成 56.8MB），是"跨盘搬运"的可行姿势
...
- 列表项的 `Etag` **就是完整的 32 位 md5**（实测：与 CDN 直链路径里嵌的那串一致，也与本地 `hashlib.md5` 算出的值一致
  —— 用它调 `upload_request` 能命中 `Reuse`）。别再当成"前 16 位"。
- **秒传命中率往往极高**：热门的考研/教程/素材资源基本都能命中，342GB 可能几分钟传完，先小样本试
- 上传前先列远端同名同大小项跳过，避免产生重复文件

## 5.1 用秒传接口做探测时的副作用（务必注意）

`upload_request` **只要 `Reuse=true` 就会立即在新位置落一份引用**（不是"仅查询"）。
所以"拿一个 etag 试一下有没有命中"这种探测**会产生真实文件**。

- 探测前想清楚：若命中，事后要 `trash` 掉探测产物
- 更安全的探测姿势：故意提交**不存在的 size**（如 `size+1`），命中不了就不会落文件；
  或用 `duplicate:2` 时先确认语义
- 实测事故：判定"某平台的 md5 是否可用"时探测了一次，命中的文件**立刻出现在根目录**，需手工清理

## 5.2 跨平台查重：不要用 md5（重要）

**哈希值是标准值，但各平台"暴露给你的字段"未必是哈希。** 实测：百度接口返回的 `md5` 字段
**第 10 位固定是非十六进制字符**（取值 `g~v`），它其实是百度内部的内容标识符
（同一串也出现在百度自己的下载直链路径里）。
...
| 拿它去调 **123 的 `upload_request`** 当 etag | ❌ `code=400 Etag格式异常` |
...
- 教训：曾用百度 md5 去和 123 比对，得出"缺失 714 个文件"的**错误结论**，改用文件名+大小后才修正

## 5.3 分页陷阱（各平台不同，都要实测）

- 百度官方 MCP `file_list`：**每页 10 条**，必须带 `page` 翻到不足一页为止；按"不足 100 条就停"写会**静默截断**
- 百度网页接口 `/api/list`：`num` 可到 1000（仍要翻页）
- 写递归遍历时**必须加看门狗**：单个请求挂死会让整个盘点卡住（实测夸克某个大目录卡了 26 分钟）；
  做法是记录"最后一次响应时间"，超时则 `proc.kill()` 解开阻塞，并把未完成的子目录记下来

## 6. 环境陷阱

- 若 `dirname/head/grep` not found、`timeout` 被解析成 Windows TIMEOUT.exe → 本会话 Git Bash 运行时坏了，
  改用 PowerShell 工具；PowerShell 输出常不回显，写文件再读（重定向是 **UTF-16**）
...
- npm 操作前 `export APPDATA='C:\Users\cgw06\AppData\Roaming'`，否则全局前缀会变成 `${APPDATA}` 字面量
- 别在个人目录乱删；改名前先备份，删前先校验 MD5 一致
- **"删除"要分平台看语义**：123 的 `trash(delete=True)` 是**彻底删除**（不可恢复）；
  夸克 `quark_delete` 走自己的回收站；百度网页接口删除进回收站（要清空才释放空间）。
  批量删除前一定先干跑 + 复核覆盖度
- **`errno:2` 有两种含义，别急着下"接口废了"的结论**：
  ① **参数错误**（最常见，见第 7 节）——百度 filemanager 对参数位置极敏感，写错一律 errno=2
  ② 系统目录/路径不存在——如百度 `/apps`（"我的应用数据"）是开放平台创建的系统目录，
     删除返回 `errno:2`；它不占空间，里面可以删空，遇到别反复重试
  ⚠ 真实教训：我曾把 ① 误判为"官方废弃 move/copy/rename"，
    实际是我参数格式写错，改对后立刻 errno=0 跑通

## 7. 百度 filemanager 接口：参数位置极敏感（errno=2 头号原因）
...
（delete 对参数位置容忍度高，所以常出现"delete 成功但 move 全挂"的假象——
别因此误判为接口废弃。）
...
### 判重判据（2026-09-14 修正，推翻旧结论）
- **百度 md5 字段不可靠**：同一份文件可能给出**不同**值。
  实测两个文件字节数完全相同(6,443,424,212)、头尾各 512KB 真 md5 也相同，
  但百度 md5 字段分别是 `fcbba42e6j…` 和 `aaea21fd0p…`。
  （该字段疑似混入上传批次/fs_id 信息，不是纯内容哈希）
- **可靠判据 = (文件名, 精确字节数)**，盘内和跨平台都用它
- **终极验证**：`Range` 下载头尾各 512KB 算真 md5 比对（6GB 文件约 25 秒，限速下亦可）
- **必须排除分卷压缩包**：`part01.rar` / `.r00` / `.z01` / `.zip.001` 每卷大小固定
  （如 838,860,800=800MB），按 size 判重会把 part01 与 part02 误判为重复，
  **删了文件就废**。正则：
  `(\.part\d+\.|\.part\d\d|\.r\d\d$|\.z\d\d$|\.\d{3}$|part\d+\.rar$|part\d+\.zip$)`
- **"版本名重合" ≠ 可删**：同版本在不同目录常是不同的包（官方原版 vs 破解版），
  字节数不同就不能当副本删。本次 9 个"重合版本"里只有 1 个是真重复。

## 4. 长跑搬运：后台进程怎么活下来（2026-09-15 血泪结论）

数百 GB 搬运要跑几十小时，**进程托管是成败关键**。踩坑顺序如下：
...
症状特征：日志在某时刻整齐断掉、无任何报错（本次是 07:03 集体消失）。
...
要点：
- **用 `pythonw.exe` 不是 `python.exe`** → 否则弹出控制台窗口（用户能看到）
- 别用 `Win32_ProcessStartup` / `ProcessStartupInformation`：实例化后
  设 `CurrentDirectory` 会报"找不到属性"，直接给 `Create` 传 `CurrentDirectory` 即可
- 返回 `ReturnValue=0` 才算成功

### 无窗口的第二道坎：子进程弹窗（**两个位置都要治**）
`pythonw` 进程虽无控制台，但它每次调 `.exe` 时，Windows 会给那个
**控制台程序**新开一个真实控制台 → 用户看到窗口闪。必须在**所有**
`subprocess` 调用上加 `creationflags`：
...
2. ❌ **守护自己的巡检调用**：`powershell.exe` 每 `--interval` 秒调一次，
   漏了就变成**黑窗每 120 秒闪一次**。这里是 loop 之外的第二个热点，
   最容易被忽略——因为守护本身是 pythonw，看不出它有控制台依赖。
...
### 改完常驻脚本必须重启才生效
改文件 ≠ 生效，正在跑的进程用的还是旧代码。用一个**只重启守护、
不碰上传循环**的小脚本（`_重启守护.py`）：按指纹杀守护 → 经 WMI 拉起新的。
⚠ 不要用 `--install` 重启：它会无条件再拉一个上传循环，造成**双跑**。

### 守护模式（推荐骨架）
`守护.py --interval N` 常驻，每 N 秒用**命令行指纹**（不是进程名）判断目标是否在跑，
不在就拉起。守护自身也用 WMI 启动。判活用 PowerShell CIM（**`wmic` 在新版 Windows 已移除**，
调用报 `WinError 2`）：
```powershell
